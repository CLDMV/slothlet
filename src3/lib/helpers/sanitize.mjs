 ...
 */
function compileGlobPattern(pattern, caseSensitive = true) {
	try {
 ...
 */
function matchesAnyPattern(input, patterns, caseSensitive = false) {
	for (const pattern of patterns) {
		if (pattern.includes("*") || pattern.includes("?")) {
			const regex = compileGlobPattern(pattern, caseSensitive);
 ...
 */
function extractPatternLiterals(pattern) {
	return pattern.split(/[*?]+/).filter(Boolean);
}

// ============================================================================
// Segment Transformation Logic
// ============================================================================
 ...
 */
function applySegmentRules(segment, index, originalString, config) {
	...
	if (matchesAnyPattern(segment, leaveRules, true)) {
		return segment;
	}

	// Rule 2: Exact preservation (case-insensitive)
	if (matchesAnyPattern(segment, leaveInsensitiveRules, false)) {
		return segment;
	}
	...
	let transformed = applyWithinSegmentPatterns(segment, upperRules, lowerRules);
 ...
 * @returns {string} Transformed segment
 */
function applyWithinSegmentPatterns(segment, upperRules, lowerRules) {
	let result = segment;
...
export function sanitizePropertyName(input, options = {}) {
	...
	const processedPrimarySegments = primarySegments.map((primarySeg) => {
		...
		const processedParts = parts.map((part, partIdx) => {
			...
			return applySegmentRules(cleanSeg, 0, originalString, config);
		});
	...
	const camelCasedSegments = processedPrimarySegments.map((seg, idx) => {
		...
		const matchesLeave = matchesAnyPattern(seg, leaveRules, true);
		const matchesLeaveInsensitive = matchesAnyPattern(seg, leaveInsensitiveRules, false);
		const matchesUpper = matchesAnyPattern(seg, upperRules, false);
		const matchesLower = matchesAnyPattern(seg, lowerRules, false);
