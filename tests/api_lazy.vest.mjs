...
import slothletLazy from "../src/slothlet.mjs?_slothlet=lazy";
...
const bound = await slothletLazy({
	lazy: true,
	dir: "./api_test",
	api_mode: "function",
	...
	describe(section.section, () => {
		for (const call of section.calls) {
			test(call.label, async () => {
				const result = await callNestedFunction(bound, call.path, call.args, true);
