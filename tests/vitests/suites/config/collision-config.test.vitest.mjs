 ...
 * 
 * Tests both contexts:
 * - collision.initial: During buildAPI (initial load) using overwrite-test files
 * - collision.addApi: During api.add() operations using additional-math.mjs
 *
...
describe.each(MATRIX_CONFIGS)("Collision Config - $name", ({ config }) => {
	...
	describe("collision.initial modes", () => {
		it("merge mode: should allow later files to overwrite during buildAPI", async () => {
			...
			// overwrite-test-2.mjs loads after overwrite-test-1.mjs (alphabetically)
		...
		it("skip mode: should silently keep first file during buildAPI", async () => {
			...
			// overwrite-test-1.mjs loads first (alphabetically)
	...
	describe("collision.addApi modes", () => {
		it("merge mode: should preserve original and add new properties", async () => {
			...
			// Add additional-math.mjs content to math path - should merge
		...
		it("replace mode: should completely replace existing value", async () => {
			...
			// Replace with additional-math.mjs (using forceOverwrite for replace mode)
			await api.slothlet.api.add("math", `${config.dir}/additional-math.mjs`, {}, {
	...
	describe("Collision with complex structures", () => {
		it("should merge nested properties correctly", async () => {
			...
			const additionalPath = config.dir.replace(/api_test(\/|\\)$/i, 'api_test/additional-math.mjs');
			await api.slothlet.api.add("math", additionalPath, {}, {
