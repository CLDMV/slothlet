...
describe("normalizeApiPath — array input and invalid types", () => {
	...
	it("accepts empty array and mounts at the root level (dot-joined produces empty string)", async () => {
		...
		await expect(api.slothlet.api.add([], TEST_DIRS.API_TEST_MIXED)).resolves.not.toThrow();
	});
	...
	it("throws when an array element is a number", async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add([123, "tools"], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it("throws when an array element is null", async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add([null, "tools"], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it("throws when an array element is an empty string", async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(["valid", ""], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});
	...
	it('throws when first element is the reserved name "slothlet"', async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(["slothlet"], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it('throws when first element is "slothlet" in a multi-segment array', async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(["slothlet", "utils"], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it('throws when single-element array is the reserved name "shutdown"', async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(["shutdown"], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it('throws when single-element array is the reserved name "destroy"', async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(["destroy"], TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});
	...
	it("throws when apiPath is a number", async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(42, TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it("throws when apiPath is a plain object", async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add({ path: "test" }, TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});

	it("throws when apiPath is a boolean", async () => {
		api = await makeApi();
		await expect(api.slothlet.api.add(true, TEST_DIRS.API_TEST_MIXED)).rejects.toThrow();
	});
});
