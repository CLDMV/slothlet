 ...
 *	@Last modified time: 2026-05-02 18:24:27 -07:00 (1777771467)
 *	-----
...
// ─── line 2833: runtimeContext fallback in permission gate ───────────────────

describe("UnifiedWrapper > applyTrap > permission runtimeContext fallback (line 2833)", () => {
	let api;
	...
	it("passes null runtimeContext to permissionManager when ctx.context is null", () => {
		const fnWrapper = resolveWrapper(api.math.add);
		const sl = fnWrapper.slothlet;
		const originalPermissionManager = sl.handlers.permissionManager;
		const originalTryGetContext = sl.contextManager.tryGetContext;

		let seenRuntimeContext = "__unseen__";

		sl.handlers.permissionManager = {
			isEnabled: () => true,
			checkAccess: (_callerPath, _targetPath, _callerFilePath, _targetFilePath, runtimeContext) => {
				seenRuntimeContext = runtimeContext;
				return true;
			}
		};

		sl.contextManager.tryGetContext = () => ({
			currentWrapper: fnWrapper,
			context: null
		});

		try {
			expect(api.math.add(1, 2)).toBe(1003);
			expect(seenRuntimeContext).toBeNull();
		} finally {
			sl.handlers.permissionManager = originalPermissionManager;
			sl.contextManager.tryGetContext = originalTryGetContext;
		}
	});
});
