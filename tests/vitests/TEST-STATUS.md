...
| suites/metadata/metadata-external-api.test.vitest.mjs | Metadata - External API | ✅ Yes (2026-01-26) | ❌ fail (2026-01-26 21:30:00) | 16/128 tests fail, 112 pass (87.5%) - Tests external metadata API (api.slothlet.metadata.set/remove/setGlobal). **FIXED**: Tests were using non-existent functions (rootMath.subtract, config.settings.getPluginConfig). Changed to use existing functions (rootMath.multiply, nested.date.today). **REMAINING BUGS** (16 failures = 2 unique tests x 8 configs): (1) 'should apply global metadata to newly added functions' - global metadata not applying to dynamically added APIs (2) 'should handle multiple global metadata updates' - metadata snapshots not preserving old values. Both are REAL implementation bugs in setGlobal() behavior. Related to metadata-tagging.md |
