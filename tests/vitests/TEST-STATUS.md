...
| suites/metadata/metadata-api-manager.test.vitest.mjs | Metadata - API Manager | ✅ Yes (2026-01-25) | ❌ fail (2026-01-25 12:33:42) | 144/160 tests fail, 16 pass - Uses TEST_DIRS.API_SMART_FLATTEN (not API_TEST_MIXED to avoid CJS bug). Tests metadata with api.add/remove. Issues: (1) 128 tests fail with "Cannot read properties of undefined" - tests reference non-existent API structure (mathEsm/mathCjs don't exist in smart_flatten folder, should use config.settings.getPluginConfig) (2) 16 tests fail with "INVALID_CONFIG_API_PATH_INVALID" on api.remove() calls. Related to metadata-tagging.md |
| suites/metadata/metadata-edge-cases.test.vitest.mjs | Metadata - Edge Cases | ✅ Yes (2026-01-25) | ❌ fail (2026-01-25 12:34:03) | 112/224 tests fail, 112 pass - Uses TEST_DIRS.API_SMART_FLATTEN. Tests edge cases: root contributor, deep nesting, special chars, large objects. Issues: (1) 96 tests fail with "Cannot read properties of undefined" or "is not a function" - tests reference non-existent API paths (2) 16 tests fail with empty/mixed export tests referencing wrong structures. Related to metadata-tagging.md |
| suites/metadata/metadata-external-api.test.vitest.mjs | Metadata - External API | ✅ Yes (2026-01-25 14:14) | ❌ fail (2026-01-25 14:14:00) | 112/256 tests fail, 144 pass (56.3%) - Tests external metadata API (api.slothlet.metadata.set/remove/setGlobal). ✅ FIXED: System metadata protection now properly enforced via merge order (system metadata applied LAST in getMetadata() merge). Issues: (1) 112 tests fail - api.config.settings path doesn't exist in TEST_DIRS.API_TEST, need to use valid paths. Related to metadata-tagging.md |
| suites/metadata/metadata-reload.test.vitest.mjs | Metadata - Hot Reload | ✅ Yes (2026-01-25) | ❌ fail (2026-01-25 12:34:34) | 120/192 tests fail, 72 pass - Uses TEST_DIRS.API_SMART_FLATTEN. Tests metadata with api.slothlet.reload() and api.slothlet.api.reload(). Issues: (1) 112 tests fail with "INVALID_CONFIG_RELOAD_NOT_IMPL" - reload not implemented yet (expected) (2) 8 tests fail with "Cannot read properties of undefined" - mathEsm references wrong API structure. Related to metadata-tagging.md. UPDATED: Added external metadata API tests (set/remove/setGlobal with reload). |
| suites/metadata/system-metadata.test.vitest.mjs | Metadata - System | ✅ Yes (2026-01-25) | ❌ fail (2026-01-25 12:43:27) | 56/288 tests fail, 232 pass - Uses TEST_DIRS.API_SMART_FLATTEN with config.settings.getPluginConfig structure. Tests immutable system metadata (moduleID, filePath, apiPath, sourceFolder). ✅ FIXED: materialize() now works with lazy proxies (changed from passing function reference to api+path string). Issues: (1) 48 tests fail - "Added API System Metadata" and "Nested Structure System Metadata" tests reference API paths that may not exist in test folder structure (2) 8 tests fail - lazy mode materialization tests have __metadata access issues. Related to metadata-tagging.md |
| suites/metadata/user-metadata.test.vitest.mjs | Metadata - User | ✅ Yes (2026-01-25) | ❌ fail (2026-01-25 12:33:13) | 208/240 tests fail, 32 pass - Uses TEST_DIRS.API_SMART_FLATTEN and metadataTestHelper for metadata API tests. Tests user metadata via add(), init, merge, metadata API (self.slothlet.metadata.*). Issues: (1) 176 tests fail with API structure mismatches - tests still reference non-existent functions/paths from old test data (2) 32 tests fail with "self.slothlet.metadata.get/caller is not a function" - metadata API not available in all configs. Related to metadata-tagging.md |
