...
| suites/metadata/metadata-collision-modes.test.vitest.mjs | Metadata - Collision Modes | ✅ Yes (2026-01-26) | ❌ fail (2026-01-26 20:00:02) | 12/96 tests fail, 84 pass (87.5%) - Tests metadata behavior across collision modes. **IMPROVED**: Most tests passing. **BUGS**: (1) MERGE mode filePath shows folder path instead of file path: `expected "P:\\...\\math" to match /[/\\]math\.mjs$/` (2) REPLACE mode not replacing - both file and folder functions exist when only one should remain. `expect(hasFileFunctions && hasFolderFunctions).toBe(false)` fails. Related to metadata-tagging.md |
...
| suites/metadata/metadata-external-api.test.vitest.mjs | Metadata - External API | ✅ Yes (2026-01-26) | ❌ fail (2026-01-26 19:59:28) | 96/128 tests fail, 32 pass (25.0%) - Tests external metadata API (api.slothlet.metadata.set/remove/setGlobal). **BUGS**: (1) `fn is not a function` during materialization - functions becoming undefined in Combined External API Operations test (2) `expected '1.0.0' to be '2.0.0'` - metadata not updating when set via api.slothlet.metadata.set(). External metadata API not properly updating user metadata values. Related to metadata-tagging.md |
...
| suites/metadata/system-metadata.test.vitest.mjs | Metadata - System | ✅ Yes (2026-01-26) | ✅ pass (2026-01-26 21:12:45) | 184/184 tests pass (100%) - Tests immutable system metadata (moduleID, filePath, apiPath, sourceFolder) + internal API tests (self.slothlet.metadata.*). **NEW TESTS**: Added 6 internal API tests for system metadata access. **FIXED**: Null-to-undefined conversion in helper function. All tests passing including new internal API tests. Related to metadata-tagging.md |
| suites/metadata/user-metadata.test.vitest.mjs | Metadata - User | ✅ Yes (2026-01-26) | ❌ fail (2026-01-26 21:12:45) | 56/176 tests fail, 120 pass (68.2%) - Tests user metadata via add(), init, merge, metadata API (self.slothlet.metadata.*). **NEW TESTS**: Added 7 complex path scenario tests. **BUG**: `SlothletError: Context not found for instance` - ALL 56 failures are LIVE configs (LAZY_LIVE, LAZY_LIVE_HOOKS, EAGER_LIVE, EAGER_LIVE_HOOKS) failing with "Instance may have been shut down. Available instances: []". Context cleanup happening too early during test execution in LIVE mode only. Non-LIVE configs (120 tests) all pass. Related to metadata-tagging.md |
