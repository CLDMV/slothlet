...
| suites/metadata/metadata-external-api.test.vitest.mjs | Metadata - External API | ✅ Yes (2026-01-26) | ✅ pass (2026-02-18) | 248/248 tests pass (100%) - Added `setForPath(apiPath, key, val)` and `removeForPath(apiPath, key?)` APIs. All functions under the path inherit path-level metadata. 96 new tests across 2 new describe blocks. |
