...
// TODO(v3): Verify reference identity expectations against v3 hot reload semantics.
...
describe.each(HOT_RELOAD_MATRIX)("Hot Reload Reference Identity - $name", ({ config }) => {
	...
	it("preserves nested object and function references across reloadApi()", async () => {
		api = await createApiInstance(config);
		await api.slothlet.api.add({ apiPath: "deep", folderPath: TEST_DIRS.API_TEST, options: { moduleId: "deep-module" } });
	...
	it("preserves mixed export references (default + named) across reloadApi()", async () => {
		api = await createApiInstance(config);
		await api.slothlet.api.add({ apiPath: "extra", folderPath: TEST_DIRS.API_TEST, options: { moduleId: "test-module" } });
	...
	it("preserves all reference types across reloadApi() in comprehensive test", async () => {
		api = await createApiInstance(config);
		await api.slothlet.api.add({ apiPath: "comprehensive", folderPath: TEST_DIRS.API_TEST, options: { moduleId: "comp-module" } });
