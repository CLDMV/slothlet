...
function createPgPoolSimulation() {
	...
	for (let i = 0; i < 5; i++) {
		...
		const responseCallback = (_, __, ___) => {
...
async function testThirdPartyCleanup() {
	...
	const totalPreShutdown = 
		preSlothletEmitters.reduce((sum, e) => sum + e.listenerCount('error') + e.listenerCount('response') + e.listenerCount('connect') + e.listenerCount('end'), 0) +
		postSlothletEmitters.reduce((sum, e) => sum + e.listenerCount('error') + e.listenerCount('response') + e.listenerCount('connect') + e.listenerCount('end'), 0) +
	...
	const preRemaining = preSlothletEmitters.reduce((sum, e) => sum + e.listenerCount('error') + e.listenerCount('response') + e.listenerCount('connect') + e.listenerCount('end'), 0);
	const postRemaining = postSlothletEmitters.reduce((sum, e) => sum + e.listenerCount('error') + e.listenerCount('response') + e.listenerCount('connect') + e.listenerCount('end'), 0);
