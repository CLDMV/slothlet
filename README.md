...
- **`defineProperty` trap invariant fixes** — three correctness bugs in the version dispatcher proxy's `defineProperty` trap fixed: `Object.defineProperty` → `Reflect.defineProperty` to propagate rejections without throwing; raw target mirrored before versioned wrapper write; over-broad guard on already-sealed property redefines removed to prevent silent partial mutation
