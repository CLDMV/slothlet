...
- **`self.X = …` actually works** — runtime assignments to `self` from inside Slothlet modules were silently dropped (proxy default-set onto an empty literal target). They now persist, get a `UnifiedWrapper` when the value is callable or object-shaped, are validated against the writer's owned namespace (a module owns its mount-point subtree; writes outside throw `LOOSE_SET_NOT_OWNED`), and survive both `api.slothlet.api.reload(apiPath)` and full `api.slothlet.reload()`.
