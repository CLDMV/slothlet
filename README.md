...
- **Fix** — a function carried on a `run()` / `scope()` context now survives the clone (shared by reference) instead of being flattened to `{}`; data on the context is still deep-copied for isolation (#408).
