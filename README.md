...
### Latest: v3.20.0 (September 2026)

- **Resource-scoped permission conditions** — permission-rule function conditions now receive a second argument, `condition(ctx, { args, target })`, carrying the call's own arguments and the concrete target path. A rule can authorize on the resource named in the call itself (`(ctx, { args }) => hasRole(ctx.caller, args[0], "read")`) instead of injecting the id into the runtime context at every call site. `callMeta` is provided at the call, construct, and captured-reference gates and is `null` elsewhere (reads, hooks, events, silent queries); additive and backward-compatible — existing one-argument conditions are unchanged (#455).
- [View full v3.20.0 Changelog](https://github.com/CLDMV/slothlet/blob/master/docs/changelog/v3/v3.20.0.md)
