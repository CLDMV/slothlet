...
/**
 * Rewrite the core export map's default `types` conditions (./types/dist/X) into paths rooted in
 * the standalone types package (./X). Entries without a dist `types` condition are dropped.
 * @internal
 */
function computeTypesExports(coreExports) {
	const prefix = "./types/dist/";
	const out = { "./package.json": "./package.json" };
	for (const [key, value] of Object.entries(coreExports || {})) {
		if (!value || typeof value !== "object") continue;
		const types = value.types;
		if (typeof types === "string" && types.startsWith(prefix)) {
			out[key] = { types: "./" + types.slice(prefix.length) };
		}
	}
	return out;
}
...
const RULES = {
	...
	"slothlet-types": {
		stage(_core, { outDir }) {
			...
			return copyDeclarations(srcDir, outDir);
		},
		compute(core) {
			// NOTE: production side only. How a consumer's TypeScript resolves these types across the
			// package boundary (stub re-export vs optional peer dep) is the open question in #146.
