...
async function buildExports() {
	...
	try {
		...
		// types/index.d.mts is generated by tsc (build:types). For source shapes where tsc
		// emits `slothlet` ONLY as the default export, `import { slothlet }` fails to
		// type-check, so we patch the named export back in here after tsc runs.
		//
		// The patch must NOT fire when tsc already emits a named `slothlet` export. tsc
		// renders the JSDoc-annotated entry point as `export function slothlet(...)` —
		// that declaration IS the named export. Appending `export { slothlet }` on top of
		// it declares `slothlet` twice and fails declaration type-checking with TS2484.
