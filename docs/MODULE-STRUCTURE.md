...
- **Framework-reserved names** (`_materialize`, `_impl`, `__impl`, …) may not be used as module file or export names, because they collide with the framework's own wrapper handles. A reserved **file** name is refused by the directory scan (`MODULE_RESERVED_FILENAME`) — on disk and in a browser manifest alike, and for the files a given mount actually loads, so a single-file `api.add` is not blocked by a reserved-name sibling it filters out. A reserved **export** name is refused when the module is read, which is at boot in eager mode and at first touch in lazy mode (`MODULE_RESERVED_EXPORT`). Either way the refusal is a named error, never a silent drop. (`__`-prefixed files were already hidden; the rejection covers the single-underscore reserved names that would otherwise load.) `slothlet.defaults.reservedExports` exposes the exact frozen `Set` of these names.
- **Configured [routine](LIFECYCLE.md#routines) names** (`initialize` and `shutdown` by default) are a related but distinct special-name category — a module may export one freely, side by side with ordinary leaves. Rather than the refusal above, every module contributing the same routine name at the same composed api path is stacked into one callable that runs each contributor in registration order, instead of an ordinary leaf's recursive-merge silently keeping only the first writer.
