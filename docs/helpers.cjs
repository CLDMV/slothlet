...
const helper = {
	...
	// Convert ID to anchor name (replace : with _)
	generateAnchor(id) {
		if (!id) return "";
	...
	generateTypedefAnchor(moduleId, typedefName) {
		...
		// Strip "module:" prefix if present, but preserve original format like function anchors
		const cleanModuleId = moduleId.startsWith("module:") ? moduleId.slice(7) : moduleId;
		return `typedef_module_${cleanModuleId}_${typedefName}`;
	},
...
const partials = {
	...
	exportedConstantsBlock(doclets, item, baseModuleName, moduleName, output) {
		...
		exportedConstants.forEach((constant) => {
			output += `\n* * *\n\n`;
			output += `<a name="${helper.generateAnchor(item.doc?.id || item.id)}.${constant.name}"></a>\n\n`;
	...
	globalTypedefDefinitions(doclets, _) {
		...
		typedefs.forEach((typedef) => {
			// Generate global typedef anchor (no module prefix)
			const anchor = `typedef_${typedef.name}`;
	...
	integratedTOC(doclets, baseModuleLongname, _) {
		...
		mainFunctionsFromHierarchy.forEach(([_, item]) => {
			...
			const anchor = helper.generateAnchor(func.id) || `${baseModuleName.replace(/\./g, "_")}.${func.name}`;
		...
		sortedItems.forEach(([path, item]) => {
			...
			if (item.type === "namespace") {
				...
				const anchorId = `module_${baseModuleName.replace(/\./g, "_")}_${item.path.join("_")}`;
				...
				childModules.forEach(([childPath, childItem]) => {
					...
					moduleFunctions.forEach((func) => {
						...
						const returns = func.returns ? ` ⇒ ${helper.formatReturnsForTOC(func.returns)}` : "";
						output += `${funcIndent}* [.${func.name}(${helper.formatParamsForTOC(func.params)})](#${childAnchorId}.${
							func.name
						})${returns}\n`;
					});
				});
				...
				moduleFunctions.forEach((func) => {
					...
					const returns = func.returns ? ` ⇒ ${helper.formatReturnsForTOC(func.returns)}` : "";
					output += `${funcIndent}* [.${func.name}(${helper.formatParamsForTOC(func.params)})](#${anchorId}.${func.name})${returns}\n`;
				});
				...
				moduleConstants.forEach((constant) => {
					...
					const typeInfo = constant.type && constant.type.names ? ` : <code>${constant.type.names.join(" | ")}</code>` : "";
					output += `${constIndent}* [.${constant.name}](#${anchorId}.${constant.name})${typeInfo}\n`;
				});
		...
		sortedItems.forEach(([path, item]) => {
			if (item.type === "main-function") {
				...
				if (isMainCallable && (func.name === baseModuleName || func.longname === baseModuleLongname || func.alias === baseModuleLongname)) {
					...
					output += `<a name="${helper.generateAnchor(func.id)}"></a>\n\n`;
					const headingLevel = "##";
					const returns = func.returns ? ` ⇒ ${helper.formatReturnsForTOC(func.returns)}` : "";
				...
				output += `<a name="${helper.generateAnchor(item.doc.id)}"></a>\n\n`;
				...
				if (moduleFunctions.length > 0) {
					moduleFunctions.forEach((func) => {
						...
						output += `\n* * *\n\n`;
						output += `<a name="${helper.generateAnchor(item.doc.id)}.${func.name}"></a>\n\n`;
				...
				if (moduleConstants.length > 0) {
					moduleConstants.forEach((constant) => {
						output += `\n* * *\n\n`;
						output += `<a name="${helper.generateAnchor(item.doc.id)}.${constant.name}"></a>\n\n`;
				...
				const anchorId = `module_${baseModuleName}_${path.replace(/\./g, "_")}`;
				const headingLevel = "#".repeat(Math.min(3 + item.level - 1, 6)); // Start at h3
				...
				childConstants.forEach(([_, childItem]) => {
					...
					output += `<a name="${helper.generateAnchor(childItem.doc.id)}"></a>\n\n`;
					...
					functions.forEach((func) => {
						output += `\n* * *\n\n`;
						output += `<a name="${helper.generateAnchor(childItem.doc.id)}.${func.name}"></a>\n\n`;
