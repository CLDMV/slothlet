...
- **[Metadata Path API & Reload-with-Metadata](./metadata-path-api-and-reload-metadata.md)** - New path-based metadata APIs and reload enhancements
  - `api.slothlet.metadata.setForPath(apiPath, key, val)` — tag all functions under a path without needing references
  - `api.slothlet.metadata.removeForPath(apiPath, key?)` — remove path-level metadata keys
