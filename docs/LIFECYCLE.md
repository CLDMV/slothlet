...
A routine's `name` is resolved **relative to a mount point** by default — a mount point being either a top-level entry from the initial `dir` scan, or the target of an explicit `api.slothlet.api.add(mountPath, ...)` call. This, not "anywhere in the tree," is the default matching scope:
...
| Dotted, mount-relative (`recursive: false`, default), e.g. `"admin.initialize"` | `<mount>.admin.initialize` exactly                                                                                                                                                        | That one mount's subtree, to resolve the fixed relative path                     |
| Mount-relative, `recursive: true`                                               | The name at any depth within each mount's own subtree                                                                                                                                     | That mount's entire subtree (depth is unbounded)                                 |
| Root-anchored (`^` prefix), e.g. `"^ext.*.initialize"`                          | The full absolute api path, crossing mount boundaries, compiled via the same glob engine hooks/permissions use (`compilePattern()`, `helpers/pattern-matcher.mjs` — `*`, `**`, `{}`, `!`) | The entire composed tree (not mount-scoped at all)                               |
