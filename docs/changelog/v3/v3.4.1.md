...
- 17 test cases × 8 matrix configurations = **136 tests**
- Covers: inter-module `addRule` / `removeRule` block, descriptor-bypass hardening, invocation-time re-check for cached callable references (both direct and descriptor paths), frozen `materialize` accessor gating, configurable accessor descriptor re-check, namespace read denial, `metadata.setGlobal` block, `i18n.setLanguage` block, primitive route (`slothlet.version`) block, an explicit external-allow test confirming that default-deny rules do not block external `api.slothlet.*` access, and runtime context propagation into `enforceInternalPermission`.
