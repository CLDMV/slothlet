...
- Redundant per-method `enforceInternalPermission()` calls have been removed; enforcement now lives in the recursive proxy layer, primarily via `get` and also via `apply`, `construct`, and `getOwnPropertyDescriptor` traps, with wrapped accessors where needed to prevent descriptor/caching bypasses.
...
- Covers a matrix of regression scenarios across supported permission configurations.
