...
3. **Hooks auto-pin caller identity** — `api.slothlet.hook.on()` now pins the
...
4. **`run`/`scope` callbacks keep caller identity** — `api.slothlet.run()` and
   `.scope()` (and `context.run`/`context.scope`) previously dropped the caller
   identity inside the callback; it is now carried through.

**Compatibility.** The `lockCaller`/`bind` additions (#1, #2) and the `run`/`scope`
fix (#4) are fully backward compatible. The hook auto-pin default (#3) is a
**behavior change**, not a pure addition: a hook registered from *inside a module*
now runs its handler under that module's caller identity instead of under the
caller that triggered the intercepted call. Handlers that do not call `self.*` or
otherwise depend on caller identity are unaffected. A handler that *intentionally*
relied on the triggering caller's identity or permissions must now opt out with
`{ lockCaller: false }`. (`before`-hook handlers previously ran with no slothlet
context at all — a `self.*` call inside one threw — so for those this default is
strictly an improvement.) Hooks registered outside a module are unaffected.
...
context managers — async ([`context-async.mjs`](../../../src/lib/handlers/context-async.mjs))
...
this release a hook handler therefore ran under the *triggering* caller's identity
(and a `before`-hook handler ran with no slothlet context at all, so a `self.*`
call inside it threw). `hook.on()` now pins the **registering module's** caller
identity onto the handler by default — the same mechanism as `lockCaller`:
