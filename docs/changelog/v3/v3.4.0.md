...
This enables per-request routing scenarios where the same caller/target pair should be allowed or denied based on runtime context values (service tier, role, domain, etc.), not just which module made the call. Conditions are plain objects (all keys must match via strict equality) or functions (called with the runtime context, truthy return = match).
...
const api = await slothlet({
	...
	permissions: {
		defaultPolicy: "deny",
		rules: [
			// Only allow premium-tier requests
			{ caller: "callers.**", target: "payments.**", effect: "allow", condition: { tier: "premium" } },
			// Explicitly deny free-tier requests
			{ caller: "callers.**", target: "payments.**", effect: "deny",  condition: { tier: "free" } }
		]
	}
});

// Allow: context matches condition
await api.slothlet.context.run({ tier: "premium" }, async () => {
	await api.callers.paymentsCaller.callCharge(100); // ✅ allowed
});

// Deny: context matches the deny condition
await api.slothlet.context.run({ tier: "free" }, async () => {
