...
This enables per-request routing scenarios where the same caller/target pair should be allowed or denied based on runtime context values (service level, role, domain, etc.), not just which module made the call. Conditions are plain objects (all keys must match via strict equality) or functions (called with the runtime context, truthy return = match).
