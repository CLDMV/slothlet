...
- **Performance benchmarks backfilled** — per-version docs for v3.4.0–v3.10.0 (+ v3.9.2) re-run on current hardware; [cross-version-summary.md](../../performance/cross-version-summary.md) gains a current-hardware era and [performance/README.md](../../performance/README.md) a startup mermaid across all releases. Documents why lazy startup ≈ eager on root-leaf-heavy fixtures (lazy loads all root-level leaves at init; only nested subtrees defer).
