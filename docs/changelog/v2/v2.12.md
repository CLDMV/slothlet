...
**Key Benefits**:
- ✅ **Reference Preservation**: Consumer code doesn't need reassignment after reload
- ✅ **Auto-Cleanup**: Prevents orphan functions when reloading modules
- ✅ **Bidirectional Tracking**: Module ownership system tracks all API paths per module
- ✅ **Cache Isolation**: New instanceId on each reload prevents stale module caches
...
// api reference still valid - no reassignment needed! ✅
```
...
// ✅ Rule 13: Old "my-plugin" paths automatically removed BEFORE loading v2
// ✅ New v2 functions replace them cleanly
// ✅ No orphan functions from v1 left behind
...
- Execution order: Path resolution → Rule 12 check → Auto-cleanup → Load new modules
...
**Key Benefits**:
- ✅ **Full Rollback Chain**: Tracks core → v1 → v2 ownership history
- ✅ **Automatic Restoration**: Removing v2 automatically restores v1 implementation
- ✅ **Core Function Tracking**: Initial slothlet functions tracked as "core" for complete rollback
- ✅ **Safe Removal**: Removing all modules restores original slothlet functionality
...
// Remove v2 → Automatically rolls back to v1 implementation
await api.removeApi({ moduleId: "plugin-v2" });
// api.myFunc restored to plugin-v1's implementation
// Ownership: ["core", "plugin-v1"]

// Remove v1 → Automatically rolls back to core implementation
...
- Full rollback chain: core → v1 → v2 → v1 → core
...
**Execution Order**: Within each hook type → subset (before/primary/after) → priority (highest first) → registration order
...
await api.shutdown();
// ✅ ALS instances disabled and removed from registry
// ✅ boundapi.__ctx property deleted
// ✅ All runtime state cleaned up

// Cleanup automatically called in reload()
await api.reload();
// ✅ Old ALS cleaned up before creating new one
// ✅ Prevents memory leaks on repeated reloads
// ✅ Safe for production hot reload scenarios
```
...
await api.shutdown();
await api.shutdown();  // ✅ No error, silently succeeds
await api.reload();    // ✅ Works even after shutdown
```
...
// ❌ Bug: Path resolved from src/lib/runtime/runtime-asynclocalstorage.mjs
// ❌ Result: 'src/api_tests/api_test_mixed' (wrong location)
```
...
**Benefits**:
- ✅ Correctly identifies user code vs slothlet internals
- ✅ Cross-platform compatible (Windows/Linux/macOS)
- ✅ Package-scoped detection prevents false positives
- ✅ Removed fragile `includes()` pattern
...
|------|--------|-------|-------------|
| **Lazy** | 1.16μs | 1.14μs | 2% faster |
| **Eager** | 1.31μs | 1.23μs | **6% faster** |
...
```javascript
// ✅ Matches caughtErrorsIgnorePattern: "^(_|___.*)$"
try {
  await api.someFunc();
} catch (_) {
...
**Migration Status**:
- ✅ **31/31 tests migrated** (100% completion)
- ✅ **2 tests intentionally skipped** (CJS-only, entry-points - documented rationale)
- ✅ **38 obsolete files removed** (11,656 lines of legacy test code)
- ✅ **4,194 total tests** passing across all matrix configurations

**Test Organization**:

```text
tests/vitests/suites/
├── addapi/               # AddApi functionality (3 test files)
├── api/                  # API structure tests (9 test files, eager/lazy split)
├── context/              # Context isolation & ALS (5 test files)
├── diagnostics/          # Mixed diagnostic scenarios
├── hooks/                # Hook system (12 test files, comprehensive coverage)
├── hot-reload/           # Hot reload functionality (4 test files)
├── isolation/            # Multi-instance & TV config (2 test files)
├── listeners/            # Event listener cleanup (2 test files)
├── metadata/             # Metadata API tests
├── ownership/            # Module ownership & removal
├── proxies/              # Proxy baseline tests
├── reference/            # Reference readonly properties
├── rules/                # API rules (Rule 12, etc.)
├── sanitization/         # Filename sanitization
└── smart-flattening/     # Smart flattening cases (4 test files)
```
...
**Capabilities**:
- ✅ **True Concurrent Worker Pool**: `Promise.race()` for optimal parallelism
- ✅ **ANSI-Preserving Error Display**: Full Vitest formatting maintained
- ✅ **Skip Test Parsing**: Reports skipped tests with reasons
- ✅ **Start Time Display**: HH:MM:SS format for execution tracking
- ✅ **Dual Duration Tracking**: Wall-clock vs sum of test times
- ✅ **Memory Reporting**: Script heap + RSS for resource monitoring
- ✅ **Proper Exit Codes**: 0 for success, 1 for failures (CI/CD ready)
...
```bash
╔══════════════════════════════════════════════════════════════╗
║                VITEST TEST RUNNER - STARTED                  ║
║  Start Time: 14:25:30                                        ║
╚══════════════════════════════════════════════════════════════╝

[1/56] ✅ PASS (2.3s) - api/eager/api-eager-basic.test.vitest.mjs
[2/56] ✅ PASS (1.8s) - api/lazy/api-lazy-basic.test.vitest.mjs
...
[56/56] ✅ PASS (3.2s) - smart-flattening/edge-cases.test.vitest.mjs

╔══════════════════════════════════════════════════════════════╗
║                    ALL TESTS COMPLETE                        ║
║  Total: 4,194 passed, 0 failed                               ║
║  Wall Clock: 171.2s | Test Time: 156.8s                      ║
║  Memory: 45.2MB heap, 178.4MB RSS                            ║
╚══════════════════════════════════════════════════════════════╝
```
...
api/
├── eager/
│   ├── api-eager-basic.test.vitest.mjs      # Core eager mode
│   ├── api-eager-hooks.test.vitest.mjs      # Eager + hooks integration
│   ├── api-eager-hot.test.vitest.mjs        # Eager + hot reload
│   └── api-eager-live.test.vitest.mjs       # Eager + live bindings
└── lazy/
    ├── api-lazy-basic.test.vitest.mjs       # Core lazy mode
    ├── api-lazy-hooks.test.vitest.mjs       # Lazy + hooks integration
    ├── api-lazy-hot.test.vitest.mjs         # Lazy + hot reload
    └── api-lazy-live.test.vitest.mjs        # Lazy + live bindings
```
...
- Full 20-configuration matrix testing (5 mode/runtime combinations × 4 hook configs)
...
**Key Points**:
- ✅ Auto-cleanup applies to **same-module reloads** (same `moduleId`)
- ✅ Happens **before** module loading (Rule 13 executes before Rule 12 checks)
- ❌ Cross-module overwrites still require `allowApiOverwrite: true` (Rule 12)
- ❌ Skipped when `mutateExisting: true` (preserves references during reload)
...
// ❌ api.plugins.featureA still exists (orphan from v1)
// ✅ api.plugins.featureC added from v2
...
await api.addApi("plugins", "./plugins/v2", { moduleId: "my-plugin" });
// ✅ api.plugins.featureA automatically removed
// ✅ api.plugins.featureB automatically removed
// ✅ api.plugins.featureC added from v2
...
| **Eager Absolute** | 1.31μs | 1.23μs | **6% faster** |
| **Lazy Absolute** | 1.16μs | 1.14μs | 2% faster |
