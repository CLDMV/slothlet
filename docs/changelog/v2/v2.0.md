...
export function esmFunction() {
	return self.cjsModule.process(); // ESM → CJS
}

// api/cjs-module.cjs
const { self } = require("@cldmv/slothlet/runtime");
function cjsFunction() {
	return self.esmModule.transform(); // CJS → ESM
}
...
- **4.3x Faster Startup**: 564.17μs vs 2.45ms in eager mode
- **Minimal Materialization Cost**: ~310μs one-time overhead
...
```plaintext
src/
├── slothlet.mjs              # Main orchestrator
└── lib/
    ├── engine/               # Experimental execution modes
    ├── modes/                # Lazy/eager loading strategies
    │   ├── slothlet_lazy.mjs
    │   └── slothlet_eager.mjs
    ├── runtime/              # Context and binding systems
    │   └── runtime.mjs
    └── helpers/              # Utilities and transformations
        ├── api_builder/      # API construction logic
        ├── multidefault.mjs  # Multi-default handling
...
1. **Folder/File Matching**: `math/math.mjs` → `api.math`
2. **Index Files**: `utils/index.mjs` → `api.utils`
3. **Single Named Export**: `config.mjs` with `export const config` → `api.config`
4. **Default Export Matching**: File name matches default function → flatten
5. **Root-Level Defaults**: Single file with root default → `api()`
...
- Startup: 564.17μs (4.3x faster)
- First call: ~310μs materialization
...
api.math.add(2, 3); // ~310μs (materialize + execute)

// Subsequent access: Direct function call
api.math.add(4, 5); // ~0.9μs (no materialization)
```
