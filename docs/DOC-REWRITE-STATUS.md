...
| `docs/v2/root/README.md` | `README.md` (in-place) | ✅ Complete | Full v3 rewrite: big v3 marketing announcement section; all examples updated to v3 API (`hook:` singular, `api.slothlet.hook.*`, `api.slothlet.context.*`, `api.slothlet.api.*`, `api.slothlet.lifecycle.on/off()`); background materialization section; lifecycle events section; collision modes and mutation controls sections; hook subsets; i18n section; full v2→v3 migration guide with breaking changes table; configuration table updated for all v3 options; mermaid diagram preserved; v2 experimental modes (VM) removed |
