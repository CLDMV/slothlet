 ...
 * @property {function(): void} destroy - Like `shutdown()` but additionally invokes registered destroy hooks before teardown. %%sig: (): void%% %%example: const api = await slothlet({ dir: './api' });|await api.destroy();%%
 * @property {function(): void} shutdown - Convenience alias for `slothlet.shutdown()`. Shuts down the instance and invokes any user-provided shutdown hook first. %%sig: (): void%% %%example: const api = await slothlet({ dir: './api' });|await api.shutdown();%%
 ...
 * @property {Function} slothlet.context.get - Get a value from the current per-request context store. %%sig: ([key]: string): *%% %%example: const api = await slothlet({ dir: './api' });|await api.slothlet.context.run({ userId: 42 }, async () => {|  const userId = api.slothlet.context.get('userId'); // 42|});%%
 * @property {function(): Object} slothlet.context.inspect - Return a snapshot of the current context state (for debugging). %%sig: (): Object%% %%example: const api = await slothlet({ dir: './api' });|await api.slothlet.context.run({ userId: 42 }, async () => {|  const snapshot = api.slothlet.context.inspect();|  // { data: { userId: 42 }, ... }|});%%
 ...
 * @property {Function} slothlet.context.scope - Execute a function with structured context options (`context`, `fn`, `args`, `merge`, `isolation`). %%sig: (options: Object): *%% %%example: const api = await slothlet({ dir: './api' });|const result = await api.slothlet.context.scope({|  context: { userId: 42 },|  fn: async () => api.myModule.getUser()|});%%
 * @property {Function} slothlet.context.set - Set a value in the current per-request context store. %%sig: (key: string, value: *): void%% %%example: const api = await slothlet({ dir: './api' });|await api.slothlet.context.run({}, async () => {|  api.slothlet.context.set('traceId', 'abc-123');|});%%
 ...
 * @property {function(): Object} [slothlet.diag.caches.get] - Get full cache diagnostic data (`{ totalCaches, caches[] }`). Only available when `diagnostics: true`. %%sig: (): Object%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const cacheData = api.slothlet.diag.caches.get();|// { totalCaches: 2, caches: [...] }%%
 * @property {function(): string[]} [slothlet.diag.caches.getAllModuleIDs] - Return all moduleIDs currently in cache. Only available when `diagnostics: true`. %%sig: (): string[]%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const ids = api.slothlet.diag.caches.getAllModuleIDs();|// ['utils/math.mjs', 'utils/strings.mjs']%%
 * @property {Function} [slothlet.diag.caches.has] - Check whether a cache entry exists for a given moduleID. Only available when `diagnostics: true`. %%sig: (moduleID: string): boolean%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const exists = api.slothlet.diag.caches.has('utils/math.mjs'); // true or false%%
 * @property {object} [slothlet.diag.context] - The `context` config value as passed to `slothlet()`.
 * @property {Function} [slothlet.diag.describe] - Describe API structure. Pass `true` to return the full API object; omit for top-level keys only. Only available when `diagnostics: true`. %%sig: ([showAll]: boolean): *%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const keys = api.slothlet.diag.describe();|const full = api.slothlet.diag.describe(true);%%
 * @property {function(): Object} [slothlet.diag.getAPI] - Return the live bound API proxy object. Only available when `diagnostics: true`. %%sig: (): Object%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const proxy = api.slothlet.diag.getAPI(); // the live bound API proxy%%
 * @property {function(): Object} [slothlet.diag.getOwnership] - Return ownership diagnostics for all registered API paths. Only available when `diagnostics: true`. %%sig: (): Object%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const ownership = api.slothlet.diag.getOwnership();%%
 * @property {object} [slothlet.diag.hook] - Hook system diagnostics sub-namespace (present only when hooks are enabled).
 * @property {function(): Object} [slothlet.diag.inspect] - Return a full diagnostic snapshot of current instance state. Only available when `diagnostics: true`. %%sig: (): Object%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const snapshot = api.slothlet.diag.inspect();|console.log(snapshot.modules, snapshot.hooks);%%
 * @property {object} [slothlet.diag.owner] - Ownership sub-namespace for diagnostics.
 * @property {Function} [slothlet.diag.owner.get] - Get the owning moduleIDs for a specific API path. Only available when `diagnostics: true`. %%sig: (apiPath: string): string[]%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const owners = api.slothlet.diag.owner.get('math.add');|// ['utils/math.mjs']%%
 * @property {object} [slothlet.diag.reference] - The `reference` config value as passed to `slothlet()`.
 * @property {function(): SlothletWarning} [slothlet.diag.SlothletWarning] - The `SlothletWarning` class — access `.captured` for warnings emitted during tests. Only available when `diagnostics: true`. %%sig: (): SlothletWarning%% %%example: const api = await slothlet({ dir: './api', diagnostics: true });|const SlothletWarning = api.slothlet.diag.SlothletWarning;|console.log(SlothletWarning.captured); // array of captured warnings%%
 * @property {object} slothlet.hook - Hook registration surface — only present when the `hook` option is enabled.
 * @property {Function} slothlet.hook.clear - Alias for `remove()`. %%sig: ([filter]: Object): void%% %%example: const api = await slothlet({ dir: './api', hook: true });|api.slothlet.hook.clear({ type: 'before' });%%
 * @property {Function} slothlet.hook.disable - Disable hooks matching a filter (empty = disable all). %%sig: ([filter]: Object): void%% %%example: const api = await slothlet({ dir: './api', hook: true });|api.slothlet.hook.disable(); // disable all|api.slothlet.hook.disable({ type: 'before' }); // disable only before hooks%%
 * @property {Function} slothlet.hook.enable - Enable hooks matching a filter (empty = enable all). %%sig: ([filter]: Object): void%% %%example: const api = await slothlet({ dir: './api', hook: true });|api.slothlet.hook.disable();|// later...|api.slothlet.hook.enable(); // re-enable all%%
 * @property {Function} slothlet.hook.list - List registered hooks matching a filter. %%sig: ([filter]: Object): Object[]%% %%example: const api = await slothlet({ dir: './api', hook: true });|const allHooks = api.slothlet.hook.list();|const beforeHooks = api.slothlet.hook.list({ type: 'before' });%%
 * @property {Function} slothlet.hook.off - Remove hooks by ID or filter object (v2 alias for `remove()`). %%sig: (idOrFilter: string|Object): void%% %%example: const api = await slothlet({ dir: './api', hook: true });|const hookId = api.slothlet.hook.on('before:math.*', handler);|api.slothlet.hook.off(hookId); // remove by ID|api.slothlet.hook.off({ type: 'after' }); // remove by filter%%
 ...
 * @property {Function} slothlet.hook.remove - Remove hooks matching a filter (`id`, `type`, `pattern`). %%sig: ([filter]: Object): void%% %%example: const api = await slothlet({ dir: './api', hook: true });|api.slothlet.hook.remove({ type: 'before', pattern: 'math.*' });%%
 * @property {object} slothlet.lifecycle - Lifecycle event emitter.
 * @property {Function} slothlet.lifecycle.off - Unsubscribe a handler from a lifecycle event. %%sig: (event: string, handler: function): void%% %%example: const api = await slothlet({ dir: './api' });|const handler = () => console.log('shutdown');|api.slothlet.lifecycle.on('shutdown', handler);|api.slothlet.lifecycle.off('shutdown', handler);%%
 ...
 * @property {function(): Object} slothlet.materialize.get - Get current materialization statistics (`{ total, materialized, remaining, percentage }`). %%sig: (): Object%% %%example: const api = await slothlet({ dir: './api', mode: 'lazy' });|const stats = api.slothlet.materialize.get();|// { total: 5, materialized: 3, remaining: 2, percentage: 60 }%%
 * @property {boolean} slothlet.materialize.materialized - `true` once all lazy folders have been fully loaded.
 * @property {function(): Promise.<void>} slothlet.materialize.wait - Returns a Promise that resolves when all lazy folders are fully materialized. %%sig: (): Promise.<void>%% %%example: const api = await slothlet({ dir: './api', mode: 'lazy' });|await api.slothlet.materialize.wait(); // resolves when all lazy modules are loaded%%
 * @property {object} slothlet.metadata - Module metadata accessor.
 * @property {function(): Object|null} slothlet.metadata.caller - Get metadata for the function that invoked the current one (runtime-injected). %%sig: (): Object|null%% %%example: const api = await slothlet({ dir: './api' });|// Inside an API module, get metadata of the calling function:|const callerMeta = api.slothlet.metadata.caller(); // null if no caller%%
 * @property {Function} slothlet.metadata.get - Get metadata for a specific function reference. %%sig: (fn: function): Object%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.set(api.math.add, 'label', 'Addition');|const meta = api.slothlet.metadata.get(api.math.add);|// { label: 'Addition' }%%
 * @property {Function} slothlet.metadata.remove - Remove per-function metadata (all keys or a specific key). %%sig: (fn: function, [key]: string): void%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.set(api.math.add, 'label', 'Addition');|api.slothlet.metadata.remove(api.math.add, 'label'); // remove key|api.slothlet.metadata.remove(api.math.add); // remove all%%
 * @property {Function} slothlet.metadata.removeFor - Remove path-level metadata for a given API path or moduleID. %%sig: (pathOrModuleId: string, [key]: string|Array.<string>): void%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.removeFor('math', 'label'); // remove 'label' from all math functions|api.slothlet.metadata.removeFor('math'); // remove all metadata%%
 * @property {function(): Object|null} slothlet.metadata.self - Get metadata for the currently-executing API function (runtime-injected). %%sig: (): Object|null%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.set(api.math.add, 'label', 'Addition');|// Inside api.math.add, get its own metadata:|const ownMeta = api.slothlet.metadata.self(); // { label: 'Addition' }%%
 * @property {Function} slothlet.metadata.set - Set per-function metadata by direct function reference. %%sig: (fn: function, key: string, value: *): void%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.set(api.math.add, 'label', 'Addition');|api.slothlet.metadata.set(api.math.add, 'tags', ['math', 'util']);%%
 * @property {Function} slothlet.metadata.setFor - Set metadata for all functions reachable under an API path or moduleID. %%sig: (pathOrModuleId: string, keyOrObj: string|Object, [value]: *): void%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.setFor('math', 'category', 'utilities');|api.slothlet.metadata.setFor('math', { category: 'utils', version: '1.0' });%%
 * @property {Function} slothlet.metadata.setGlobal - Set global metadata applied to every function in the instance. %%sig: (key: string, value: *): void%% %%example: const api = await slothlet({ dir: './api' });|api.slothlet.metadata.setGlobal('version', '2.0');|// Every function now has metadata: { version: '2.0' }%%
 * @property {object} slothlet.owner - Direct path ownership accessor (shorthand for `slothlet.ownership`).
 * @property {Function} slothlet.owner.get - Get ownership info for a specific API path. %%sig: (apiPath: string): Object%% %%example: const api = await slothlet({ dir: './api' });|const info = api.slothlet.owner.get('math.add');|// { path: 'math.add', owners: Set { 'utils/math.mjs' } }%%
 * @property {object} slothlet.ownership - Module ownership registry.
 * @property {Function} slothlet.ownership.get - Get the set of moduleIDs that own a given API path. %%sig: (apiPath: string): Set.<string>%% %%example: const api = await slothlet({ dir: './api' });|const owners = api.slothlet.ownership.get('math.add');|// Set { 'utils/math.mjs' }%%
 * @property {Function} slothlet.ownership.unregister - Unregister a module from all ownership records. %%sig: (moduleID: string): void%% %%example: const api = await slothlet({ dir: './api' });|await api.slothlet.api.remove('math');|api.slothlet.ownership.unregister('utils/math.mjs');%%
 ...
 * @property {Function} slothlet.run - Execute a callback with isolated per-request context data. Convenience alias for `slothlet.context.run()`. %%sig: (contextData: Object, callback: function, args: *): *%% %%example: const api = await slothlet({ dir: './api' });|const result = await api.slothlet.run({ userId: 42 }, async () => {|  return api.myModule.getUser();|});%%
 * @property {Function} slothlet.scope - Execute a function with full structured per-request context options. Convenience alias for `slothlet.context.scope()`. %%sig: (options: Object): *%% %%example: const api = await slothlet({ dir: './api' });|const result = await api.slothlet.scope({|  context: { userId: 42 },|  fn: async () => api.myModule.getUser()|});%%
 * @property {function(): Promise.<void>} slothlet.shutdown - Shut down the instance and release all resources. %%sig: (): Promise.<void>%% %%example: const api = await slothlet({ dir: './api' });|await api.slothlet.shutdown();%%
 */
