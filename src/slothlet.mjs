 ...
 * @property {object} slothlet.api.modules - Module discovery + mount sub-namespace. Composes subsystems shipped as separate npm packages (each with a `slothlet.module.json` manifest) into this api tree at runtime. The full surface — `discover` / `sort` / `addModule` / `addModules` / `addDiscovered` / `removeModule` / `getDiscoveryCache` / `clearDiscoveryCache` / `getStaleMounts` — is documented in [`docs/MODULE-DISCOVERY.md`](../docs/MODULE-DISCOVERY.md), including option shapes (`DiscoverOptions`, `AddModuleOptions`, `AddModulesOptions`), the `DiscoverResult` / `MountResult` / `FailureEntry` types, multi-version routing semantics, error codes, and the `modules:*` lifecycle events. %%sig: { discover, sort, addModule, addModules, addDiscovered, removeModule, getDiscoveryCache, clearDiscoveryCache, getStaleMounts }%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|await api.slothlet.api.modules.addDiscovered({ scanRoot: process.cwd(), prefix: "@cldmv/packrat-driver-" });%%
