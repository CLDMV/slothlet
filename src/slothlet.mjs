 ...
 * @property {Function} slothlet.api.modules.discover - Walk the filesystem for slothlet modules; replace the per-instance discovery cache and return the fresh results. %%sig: ([options]: Object): Promise.<DiscoverResult[]>%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|const found = await api.slothlet.api.modules.discover({ scanRoot: process.cwd(), prefix: "@cldmv/packrat-driver-" });%%
 * @property {Function} slothlet.api.modules.sort - Pure sort; default comparator is `priority` desc with `packageName` asc tiebreak. Pass a custom comparator to override. %%sig: (results: DiscoverResult[], [comparator]: function): DiscoverResult[]%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|const ordered = api.slothlet.api.modules.sort(await api.slothlet.api.modules.discover());%%
 * @property {Function} slothlet.api.modules.addModule - Mount one module by package name (cache lookup) or by DiscoverResult directly. Lazy-triggers `discover()` when the cache is empty. %%sig: (nameOrResult: string|DiscoverResult, [options]: Object): Promise.<MountResult>%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|await api.slothlet.api.modules.addModule("@cldmv/packrat-driver-opensearch");%%
 * @property {Function} slothlet.api.modules.addModules - Batch mount a list of modules. Accepts a heterogeneous `(string | DiscoverResult)[]`. Supports `onFailure: "throw" | "rollback" | "best-effort"` and `concurrency` for parallel batches; under `best-effort` returns the `{ mounted, failed }` aggregate instead of a plain array. %%sig: (items: Array.<string|DiscoverResult>, [options]: Object): Promise.<MountResult[]|Object>%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|await api.slothlet.api.modules.addModules(["@org/a", "@org/b"], { onFailure: "best-effort", concurrency: 4 });%%
 * @property {Function} slothlet.api.modules.removeModule - Unmount a previously-mounted module. %%sig: (name: string, [options]: Object): Promise.<void>%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|await api.slothlet.api.modules.removeModule("@cldmv/packrat-driver-opensearch");%%
 * @property {Function} slothlet.api.modules.addDiscovered - One-shot convenience: chains `discover()` → `sort()` → `addModules()` for the most common boot-time flow. %%sig: ([options]: Object): Promise.<MountResult[]>%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|await api.slothlet.api.modules.addDiscovered({ scanRoot: process.cwd(), prefix: "@cldmv/packrat-driver-" });%%
 * @property {Function} slothlet.api.modules.getDiscoveryCache - Snapshot of the current discovery cache. Returns the same `DiscoverResult[]` shape `discover()` produces. %%sig: (): DiscoverResult[]%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|const cached = api.slothlet.api.modules.getDiscoveryCache();%%
 * @property {Function} slothlet.api.modules.clearDiscoveryCache - Empty the discovery cache (does NOT unmount anything already mounted). %%sig: (): void%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|api.slothlet.api.modules.clearDiscoveryCache();%%
 * @property {Function} slothlet.api.modules.getStaleMounts - Reconciliation aid: returns mounts whose `${packageName}@${version}` no longer appears in the discovery cache. Useful for hot-reload prune logic. %%sig: (): MountResult[]%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api' });|const stale = api.slothlet.api.modules.getStaleMounts();%%
 ...
 * @property {boolean} slothlet.permissions.control.enabled - Current permission-enforcement state. Exposed as a getter so descriptor-based reads remain permission-gated. %%sig: boolean%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api', permissions: { defaultPolicy: 'allow' } });|const on = api.slothlet.permissions.control.enabled;%%
 * @property {function(): void} slothlet.permissions.control.enable - Enable permission enforcement globally. %%sig: (): void%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api', permissions: { defaultPolicy: 'allow' } });|api.slothlet.permissions.control.enable();%%
 * @property {function(): void} slothlet.permissions.control.disable - Disable permission enforcement globally (all calls allowed). %%sig: (): void%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api', permissions: { defaultPolicy: 'deny' } });|api.slothlet.permissions.control.disable();%%
 * @property {boolean} slothlet.permissions.control.readGatingEnabled - Current read-level gating state. `true` when terminal data-value property reads are permission-gated. Exposed as a getter so descriptor-based reads remain permission-gated. %%sig: boolean%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api', permissions: { defaultPolicy: 'allow' } });|const gated = api.slothlet.permissions.control.readGatingEnabled;%%
 * @property {function(boolean): void} slothlet.permissions.control.readGating - Enable or disable read-level permission gating at runtime. Throws `INVALID_ARGUMENT` for a non-boolean argument. %%sig: (value: boolean): void%% %%example: // ESM usage via slothlet API|import slothlet from "@cldmv/slothlet";|const api = await slothlet({ dir: './api', permissions: { defaultPolicy: 'allow' } });|api.slothlet.permissions.control.readGating(false); // stop gating reads|api.slothlet.permissions.control.readGating(true);  // resume%%
