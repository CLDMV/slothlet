	...
	async createSlothletNamespace(userApi) {
		...
		const namespace = {
			...
			metadata: {
				...
				/**
				 * @param {string} path - Dot-notation path to function (e.g., "math.add").
				 * @returns {object|null} Metadata object or null if not found.
				 * @public
				 *
				 * @description
				 * Gets metadata for a function by traversing the API using a dot-notation path.
				 * This method is designed to be called from within API functions where the
				 * runtime context (self) is available.
				 *
				 * @example
				 * // From within an API function
				 * import { self } from "@cldmv/slothlet/runtime";
				 *
				 * export function checkMetadata() {
				 *   const meta = self.slothlet.metadata.get("math.add");
				 *   return meta;
				 * }
				 */
				get: function slothlet_metadata_get(path) {
					...
					// Get the API root from context
					const ctx = slothlet.contextManager.tryGetContext();
					if (!ctx || !ctx.self) {
						throw new slothlet.SlothletError("RUNTIME_NO_ACTIVE_CONTEXT_SELF", {}, null, {
					...
					if (typeof target === "function") {
						return slothlet.handlers.metadata.getMetadata(target);
					}
				...
				/**
				 * @returns {object|null} Metadata for the currently executing function.
				 * @public
				 *
				 * @description
				 * Returns metadata for the function that is currently executing.
				 * This uses the runtime context to identify the current function.
				 *
				 * @example
				 * // From within an API function
				 * import { self } from "@cldmv/slothlet/runtime";
				 ...
				 *   const myMeta = self.slothlet.metadata.self();
				 *   console.log("My metadata:", myMeta);
				 * }
				 */
				self: function slothlet_metadata_self() {
					// Get current wrapper from context
					const ctx = slothlet.contextManager.tryGetContext();
					if (!ctx || !ctx.currentWrapper) {
						throw new slothlet.SlothletError("RUNTIME_NO_ACTIVE_CONTEXT", {}, null, {
							validationError: true,
							hint: "metadata.self() must be called from within a slothlet API function"
						});
					}

					return slothlet.handlers.metadata.getMetadata(ctx.currentWrapper);
				},

				/**
				 * @returns {object|null} Metadata for the calling function.
				 * @public
				 *
				 * @description
				 * Returns metadata for the function that called the currently executing function.
				 * Uses the call stack to identify the caller.
				 *
				 * @example
				 * // From within an API function
				 * import { self } from "@cldmv/slothlet/runtime";
				 ...
				 *   console.log("Called by:", callerMeta?.apiPath);
				 * }
				 *
				 * export function outerFunction() {
				 *   return innerFunction(); // innerFunction will see outerFunction as caller
				 * }
				 */
				caller: function slothlet_metadata_caller() {
					// Get caller wrapper from context (if available)
					const ctx = slothlet.contextManager.tryGetContext();
					...
					return slothlet.handlers.metadata.getMetadata(ctx.callerWrapper);
				}
			},
