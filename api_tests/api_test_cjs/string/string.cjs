/**
 *	@Project: @cldmv/slothlet
 *	@Filename: /api_tests/api_test_cjs/string/string.cjs
 *	@Date: 2025-09-09T08:06:19-07:00 (1757430379)
 *	@Author: Nate Corcoran <CLDMV>
 *	@Email: <Shinrai@users.noreply.github.com>
 *	-----
 *	@Last modified by: Nate Corcoran <CLDMV> (Shinrai@users.noreply.github.com)
 *	@Last modified time: 2026-03-01 20:21:13 -08:00 (1772425273)
 *	-----
 *	@Copyright: Copyright (c) 2013-2026 Catalyzed Motivation Inc. All rights reserved.
 */

/**
 * @fileoverview String API module (CJS version) for testing slothlet loader with auto-flattening behavior. Internal file (not exported in package.json).
 * @module api_test_cjs.string
 * @memberof module:api_test_cjs
 */

// CJS runtime imports
let _;
(async () => {
	({ self: _, context: _, reference: _ } = await import("@cldmv/slothlet/runtime"));
})();

/**
 * String API object with string manipulation operations for testing auto-flattening (CJS version).
 * This module tests slothlet's ability to flatten single-file folder structures.
 * Accessed as `api.string` in the slothlet API.
 * @alias module:api_test_cjs.string
 * @public
 * @example // ESM usage via slothlet API
 * import slothlet from "@cldmv/slothlet";
 * const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
 * console.log(api_test_cjs.string.upper("abc")); // 'ABC'
 * console.log(api_test_cjs.string.reverse("abc")); // 'cba'
 *
 * @example // ESM usage via slothlet API (inside async function)
 * async function example() {
 *   const { default: slothlet } = await import("@cldmv/slothlet");
 *   const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
 *   console.log(api_test_cjs.string.upper("abc")); // 'ABC'
 *   console.log(api_test_cjs.string.reverse("abc")); // 'cba'
 * }
 *
 * @example // CJS usage via slothlet API (top-level)
 * let slothlet;
 * (async () => {
 *   ({ slothlet } = await import("@cldmv/slothlet"));
 *   const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
 *   console.log(api_test_cjs.string.upper("abc")); // 'ABC'
 *   console.log(api_test_cjs.string.reverse("abc")); // 'cba'
 * })();
 *
 * @example // CJS usage via slothlet API (inside async function)
 * const slothlet = require("@cldmv/slothlet");
 * const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
 * console.log(api_test_cjs.string.upper("abc")); // 'ABC'
 * console.log(api_test_cjs.string.reverse("abc")); // 'cba'
 */
const string = {
	/**
	 * Converts a string to uppercase.
	 * @function upper
	 * @public
	 * @param {string} str - String to convert to uppercase.
	 * @returns {string} The uppercased string.
	 * @example // ESM usage via slothlet API
	 * import slothlet from "@cldmv/slothlet";
	 * const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 * console.log(api_test_cjs.string.upper("abc")); // 'ABC'
	 *
	 * @example // ESM usage via slothlet API (inside async function)
	 * async function example() {
	 *   const { default: slothlet } = await import("@cldmv/slothlet");
	 *   const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 *   console.log(api_test_cjs.string.upper("abc")); // 'ABC'
	 * }
	 *
	 * @example // CJS usage via slothlet API (top-level)
	 * let slothlet;
	 * (async () => {
	 *   ({ slothlet } = await import("@cldmv/slothlet"));
	 *   const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 *   console.log(api_test_cjs.string.upper("abc")); // 'ABC'
	 * })();
	 *
	 * @example // CJS usage via slothlet API (inside async function)
	 * const slothlet = require("@cldmv/slothlet");
	 * const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 * console.log(api_test_cjs.string.upper("abc")); // 'ABC'
	 */
	upper(str) {
		return str.toUpperCase();
	},

	/**
	 * Reverses a string.
	 * @function reverse
	 * @public
	 * @param {string} str - String to reverse.
	 * @returns {string} The reversed string.
	 * @example // ESM usage via slothlet API
	 * import slothlet from "@cldmv/slothlet";
	 * const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 * console.log(api_test_cjs.string.reverse("abc")); // 'cba'
	 *
	 * @example // ESM usage via slothlet API (inside async function)
	 * async function example() {
	 *   const { default: slothlet } = await import("@cldmv/slothlet");
	 *   const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 *   console.log(api_test_cjs.string.reverse("abc")); // 'cba'
	 * }
	 *
	 * @example // CJS usage via slothlet API (top-level)
	 * let slothlet;
	 * (async () => {
	 *   ({ slothlet } = await import("@cldmv/slothlet"));
	 *   const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 *   console.log(api_test_cjs.string.reverse("abc")); // 'cba'
	 * })();
	 *
	 * @example // CJS usage via slothlet API (inside async function)
	 * const slothlet = require("@cldmv/slothlet");
	 * const api_test_cjs = await slothlet({ dir: './api_tests/api_test_cjs' });
	 * console.log(api_test_cjs.string.reverse("abc")); // 'cba'
	 */
	reverse(str) {
		return str.split("").reverse().join("");
	}
};

module.exports = string;
